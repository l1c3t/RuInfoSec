...
https://nbviewer.jupyter.org/github/hunters-forge/ThreatHunter-Playbook/blob/master/docs/content/notebooks/windows/06_credential_access/credential_access/WIN-191030201010.ipynb
...
Я решил начать добавлять ссылки на наборы данных Mordor к каждому обнаружению. Я надеюсь, что это подтолкнёт других взглянуть на используемый набор данных и потенциально поможет мне улучшить логику обнаружения. Есть несколько событий, которые я, возможно, даже не учитывал при создании обнаружения (всегда!).
...
Для тех, кто знаком с [контейнерами Docker](https://www.docker.com/resources/what-container), все вышеперечисленные шаги можно объединить в [файл Docker (Dockerfile)](https://docs.docker.com/engine/reference/builder/). Я начал искать способы для автоматизации развёртывания, так как не все могут развернуть докер-контейнер на своём компьютере, но не смог найти простой способ поделиться своими исследованиями без каких-либо базовых знаний о Docker, vagrant или packer, AWS CloudFormation или любом другом способе развёртывания чего-либо локально или через облачного провайдера. Я искал **подход с пмощью единственного клика**. Это было правдой, пока я не прочитал о [проекте Binder](https://mybinder.readthedocs.io/en/latest/index.html) и его продукте [BinderHub](https://binderhub.readthedocs.io/en/latest/).
...
- [Repo2Docker](https://github.com/jupyter/repo2docker), который генерирует образ Docker, используя Git-репозиторий, размещенный в Интернете.
...
- **JupyterHub** для развёртывания временных контейнеров для пользователей.
### Нужно ли мне создавать свой собственный BinderHub?
Вы можете создать собственное развёртывание BinderHub и запускать код в облаке, если хотите, но команда Binder уже сделала это для вас и имеет сервер BinderHub, работающий на `*[mybinder.org](https://mybinder.org/)*` в качестве общедоступного сервиса (бесплатно). Если вы зайдёте на этот сайт, вам просто нужно указать имя или URL-адрес хранилища, в котором находится **репозиторий Binder**.

![7]()

Вы можете разместить свой репозиторий Binder в **Github**, **GitLab** и т. д., как показано ниже
...
Это файлы, используемые Binder для создания среды, необходимой для запуска вашего кода. Например, если я хочу собрать сервер Jupyter Notebook, я должен сказать Binder, как это сделать. Список всех файлов конфигурации, доступных для создания среды, см. На [странице «Файлы конфигурации»](https://mybinder.readthedocs.io/en/latest/config_files.html#config-files). Кроме того, есть несколько способов сделать это, и вы можете увидеть некоторые примеры репозиториев Binder [здесь](https://mybinder.readthedocs.io/en/latest/sample_repos.html).

Я использую [контейнеры Docker](https://www.docker.com/resources/what-container) для создания Notebook'ов Jupyter, поэтому я решил создать один, чтобы поделиться своими Notebook'ами и исследованиями с другими. У меня также есть целый проект вокруг Jupyter Notebook и контейнеров Docker под названием [Notebooks Forge](https://github.com/hunters-forge/notebooks-forge), проект, посвященный созданию и предоставлению серверов `Notebooks` для операторов `Defensive` и `Offensive` (фиолетовый пост об этом скоро).
## Репозиторий Binder для среды Threat Hunter Playbook через Docker
...
Как я упоминал ранее, я создаю серверы Jupyter Notebook с помощью файлов Docker из проекта [Notebooks Forge](https://github.com/hunters-forge/notebooks-forge) и сохраняю встроенные образы Docker в [своём собственном общедоступном реестре Docker](https://hub.docker.com/u/cyb3rward0g). Вы можете просто скачать изображения и использовать их прямо сейчас.
